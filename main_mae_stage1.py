"""Main script for stage1 (see Fig. 2) of the training pipeline.
Spectral modality adaptation training on the target domain.

Use this script to:
    * load a pre-trained RSFMs (without dinov3)
    * initialize Hyper tokenizer and AoMoA parameters
    * train RSFMs on a reconstruction objective.
"""
...
@hydra.main(version_base=None, config_path="configs/stage1", config_name="experiment_MTS12.yaml") 
def main(cfg):
    ...
    callbacks = src.utils.get_callbacks(run.dir, config.model.type)
    ...
    trainer = lightning.pytorch.Trainer(
        ...
        # [checkpoint_callback, early_stopping_callback], #these will be overridden by callbacks in the task
        logger=[wandb_logger],
