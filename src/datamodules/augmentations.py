...
class RandomRotationCustom(transforms.RandomRotation):
    def __call__(self, sample):
        ...
        return transforms.functional.rotate(sample, angle, resample=self.resample, expand=self.expand, center=self.center)

class RandomHorizontalFlipCustom(transforms.RandomHorizontalFlip):
    def __call__(self, sample):
        if self.p < 1.0 and torch.rand(1) < self.p:
...
def get_train_augmentation(size: Union[int, Tuple[int], List[int]], seg_fill: int = 0):
    return Compose([
        ...
        RandomRotation(degrees=90, p=0.5, seg_fill=seg_fill),
        # RandomRotation(degrees=180, p=0.5, seg_fill=seg_fill),
        # RandomRotation(degrees=270, p=0.5, seg_fill=seg_fill),
        # # RandomResizedCrop(size, scale=(0.5, 2.0), seg_fill=seg_fill),
        Resize(size=[size,size]),
    ])

def get_augmentation(size: Union[int, Tuple[int], List[int]]):
    return Compose([
        Resize(size=[size,size]),
    ])
...
if __name__ == '__main__':
    ...
    #     # Pad((512, 512))
    # ])
    aug = get_train_augmentation(size=[256,256])
