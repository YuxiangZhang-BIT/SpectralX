...
class MulticlassDiceLoss(nn.Module):
    ...
    def forward(self, input, target, weight=None):
        ...
        if weight is None:
            weight = torch.ones(C).to(target.device) #uniform weights for all classes
        else:
            weight = torch.tensor(weight).to(target.device)
...
class MulticlassHybridLoss(nn.Module):
    def __init__(self, ignore_index=None, reduction='mean'):
        ...
        self.ce = MulticlassCELoss(ignore_index=ignore_index, reduction=reduction)
        self.dice = MulticlassDiceLoss(ignore_index=ignore_index, reduction=reduction)
    
    def forward(self, input, target, weight=None):
        l1 = self.ce(input, target, weight)
        # return l1
        l2 = self.dice(input, target, weight)
        return l1 + l2