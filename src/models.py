...
# sat-mae-pp imports
import src.sat_mae_pp.models_mae
import src.sat_mae_pp.models_vit

# scale_mae imports
import src.scale_mae.models_mae
import src.scale_mae.models_vit
import src.scale_mae.util.pos_embed
from src.scale_mae.util.pos_embed import get_1d_sincos_pos_embed_from_grid_torch, get_2d_sincos_pos_embed_with_resolution

# dinov3 imports
import src.dinov3.models_vit

import torch.nn as nn
from src.utils import *
from src.adapters.AoMoA import AoMixtureOfAdapters
    
class HyperTposPatchEmbed(nn.Module):
    def __init__(self, img_size=224, in_channels=3, downsample_dim=512, token_num=4, 
                 out_dim=768, with_cls_token=True, wavelength=None):
        super().__init__()
        self.downsample_dim = downsample_dim
        self.downsample_feat_h = img_size // 8
        self.with_cls_token = with_cls_token
        self.downsample_module = DownsampleConv(in_channels=in_channels, out_dim=self.downsample_dim)
        self.wavelength = wavelength
        self.input_res = 10
        self.spa_semantic_module = nn.Sequential(*[
            # Local Window Attn
            SAM_Block(
        ...
        self.spe_semantic_module = nn.Sequential(*[
            # Global Window Attn
            SAM_Block(
                dim=self.downsample_feat_h**2,
                out_dim=self.downsample_feat_h**2,
                num_heads=8,
                window_size=0,
                spe=True,
            ),
        ...
        self.spa_query = nn.Parameter(torch.randn(token_num , out_dim//2))
        self.spe_query = nn.Parameter(torch.randn(token_num, out_dim//2))
        self.spa_match_attn = PerceiverAttention(self.downsample_dim, out_dim=out_dim//2, dim_head=self.downsample_dim // 2, heads=1)
        self.spe_match_attn = PerceiverAttention(self.downsample_feat_h**2, out_dim=out_dim//2, dim_head=self.downsample_feat_h**2 // 2, heads=1)
    ...
    def forward(self, x):
        ...
        spa_semantic_feat = self.spa_semantic_module(downsample_feat)
        spe_semantic_feat = self.spe_semantic_module(downsample_feat)
        spa_semantic_feat = einops.rearrange(spa_semantic_feat, "B H W d -> B (H W) d")
        spe_semantic_feat = einops.rearrange(spe_semantic_feat, "B H W d -> B d (H W)")
        
        spa_pos_embed = get_2d_sincos_pos_embed_with_resolution(
            self.downsample_dim, self.downsample_feat_h, torch.tensor([self.input_res]), device=downsample_feat.device
        )
        spa_semantic_feat = spa_semantic_feat + spa_pos_embed

        ori_coord = np.linspace(0, 1, len(self.wavelength))
        new_coord = np.linspace(0, 1, 512)  
        spe_pos_embed = get_1d_sincos_pos_embed_from_grid_torch(self.downsample_feat_h ** 2, torch.tensor(self.wavelength)*1000)
        interp_func = interp1d(ori_coord, spe_pos_embed, axis=0, kind='linear')
        spe_pos_embed_inter = torch.tensor(interp_func(new_coord), dtype=torch.float32).to(downsample_feat.device)
        spe_semantic_feat = spe_semantic_feat + spe_pos_embed_inter
        # attribute match
        spa_latents = einops.repeat(self.spa_query, 'n d -> b n d', b=spa_semantic_feat.shape[0])
        spa_tokens, _ = self.spa_match_attn(spa_semantic_feat, spa_latents) 

        spe_latents = einops.repeat(self.spe_query, 'n d -> b n d', b=spe_semantic_feat.shape[0])
        spe_tokens, _ = self.spe_match_attn(spe_semantic_feat, spe_latents)
        tokens = self.match_ff(torch.cat((spa_tokens+spa_latents, spe_tokens+spe_latents), dim=2))
        feats = {'spa_fea': spa_semantic_feat, 'spe_fea': spe_semantic_feat}
...
class HyperTLoRAdapter(torch.nn.Module):
    """SLR adapter for conv layers."""

    def __init__(self, conv2d: torch.nn.Conv2d, hidden_dim: int = 16, wavelength: list = None):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.proj = HyperTposPatchEmbed(img_size=224, in_channels=conv2d.in_channels, downsample_dim=512, token_num=196, 
                                  out_dim=conv2d.out_channels, with_cls_token=False, wavelength=wavelength)
        self.kernel_size = conv2d.kernel_size
        self.scaler = torch.nn.Parameter(torch.ones(conv2d.out_channels))
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        return torch.einsum("bdhw,d->bdhw", x, self.scaler), feats
...
class ScaledLowRankConvAdapter(torch.nn.Module):
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        # return torch.einsum("bdhw,d->bdhw", x, self.scaler), None
...
class ScaledLowRankAdapter(torch.nn.Module):
    ...
    def __init__(self, linear: torch.nn.Linear, hidden_dim: int = 16, wavelength: list = None):
        super().__init__()
...
class LowRankAdapter(torch.nn.Module):
    ...
    def __init__(self, linear: torch.nn.Linear, hidden_dim=16, wavelength: list = None):
...
class AoMoAdapter(torch.nn.Module):
    """AoMoA adapter for linear layers."""
    ...
    def __init__(self, linear: torch.nn.Linear, hidden_dim: int = 16, wavelength: list = None):
        ...
        self.blocks_MoA = AoMixtureOfAdapters(self.in_dim//2, self.out_dim//2, task_num=2)
        self.scaler = torch.nn.Parameter(torch.ones(self.out_dim))

    def forward(self, x: torch.tensor) -> torch.tensor:

        spa_feats, spe_feats = torch.tensor_split(x, [x.shape[-1]//2], dim=-1)
        with torch.cuda.amp.autocast(enabled=False):
            spa_MoA = self.blocks_MoA(spa_feats, 0) 
            spe_MoA = self.blocks_MoA(spe_feats, 1)
            x_MoA = torch.cat([spa_MoA, spe_MoA], dim=-1)
        x = self.linear(x)
        x += self.scaler*x_MoA
...
class AoMoAConfigTimmViT:
    """Config to add AoMoA adapters to a timm ViT."""

    def __init__(
        self, hidden_dim: int = 8, patch_embed: bool = False, norm: bool = True
    ):
        self.lora_rank = hidden_dim
        self.adapter_modules = ".*mlp|decoder_embed|decoder_pred"
        self.adapter_layers = "fc1|fc2|proj|decoder_embed|decoder_pred"
        # self.adapter_modules = ".*attn|.*mlp|decoder_embed|decoder_pred"
        # self.adapter_layers = "qkv|fc1|fc2|proj|decoder_embed|decoder_pred"
        if patch_embed:
            self.adapter_modules += "|patch_embed"
        ...
        if re.fullmatch(config.adapter_modules, m_name):
            ...
            for c_name, layer in children.items():
                if re.fullmatch(config.adapter_layers, c_name):
                    if adapter and isinstance(layer, torch.nn.Linear):
                        adp = adapter
                    elif adapter and isinstance(layer, torch.nn.LayerNorm):
                        adp = adapter
                        ...
                        break
                        raise ValueError()
                    adapter_instance = adp(layer, hidden_dim=config.lora_rank, wavelength=wavelength)
                    if not trainable:
                        if only_proj_trainable: 
                            if isinstance(adapter_instance, (HyperTLoRAdapter, ScaledLowRankConvAdapter, AoMoAdapter)):
                    ...
                    if set_as_module:
                        ...
                        if 'patch' in m_name or 'decoder' in m_name or int(m_name.split('.')[1]) in adp_indices:
...
def add_adapter(
    ...
    only_proj_trainable: bool = False,
    ...
    if type == "lora":
    ...
    elif type == "spectral_adaptation":
        assert shared == False
        config = AoMoAConfigTimmViT(
            hidden_dim=hidden_dim,
            patch_embed=patch_embed_adapter,
            norm=norm_trainable,
        )
        ...
        raise AttributeError(f"´type´ must be in ['lora', 'ia3', 'low-rank-scaling', 'spectral_adaptation']")
    ...
    if pretrained:
        ...
        if args.in_chans != ckpt_patch_embed_weight.shape[1]:
            if args.in_chans % ckpt_patch_embed_weight.shape[1] == 0:
                ...
                checkpoint["model"]["patch_embed.proj.weight"] = new_pe_weight
                if model_type == "mae" or model_type == "mae-moa":
                    # also change final decoder layer from 3*patch_size*patch_size
...
def get_dinov3(
    ...
    print(f"{'dinov3='}")
    model = src.dinov3.models_vit.DinoVisionTransformer(
        img_size=img_size,
        in_chans=in_chans,
        patch_size=16,
        embed_dim=1024,
    ...
    if pretrained:
        ckpt_patch_embed_weight = checkpoint["patch_embed.proj.weight"]
        if in_chans != ckpt_patch_embed_weight.shape[1]:
            if in_chans % ckpt_patch_embed_weight.shape[1] == 0:
                print(f"Rescaling pretrained patch_embed weight to fit new {in_chans=}")
                new_pe_weight = upscale_patch_embed(
                ...
                checkpoint["patch_embed.proj.weight"] = new_pe_weight
            else:
                print("Deleting pre-trained patch embedding weights")
...
def get_sat_mae_pp(
    ...
    if model_type == "mae":
        # return mae model including decoder
        model = src.sat_mae_pp.models_mae.__dict__["MaskedAutoencoderViT"](
            patch_size=args.patch_size,
            img_size=args.input_size,
            in_chans=args.in_chans,
            adapter_type=adapter_type
        ...
        model = src.sat_mae_pp.models_vit.__dict__[args.model](
            patch_size=args.patch_size,
            img_size=args.input_size,
            in_chans=args.in_chans,
            num_classes=args.nb_classes,
    ...
    if pretrained:
        ...
        if args.in_chans != ckpt_patch_embed_weight.shape[1]:
            if args.in_chans % ckpt_patch_embed_weight.shape[1] == 0:
                ...
                checkpoint["model"]["patch_embed.proj.weight"] = new_pe_weight
                del checkpoint["model"]['proj_up_norm.weight']
                del checkpoint["model"]['proj_up_norm.bias']
                del checkpoint["model"]['proj_up_conv.weight']
                del checkpoint["model"]['proj_up_conv.bias']
                del checkpoint["model"]['up_block.proj_out.weight']
                del checkpoint["model"]['up_block.proj_out.bias']
                del checkpoint["model"]['up_block.up_conv.weight']
                del checkpoint["model"]['up_block.up_conv.bias']
                del checkpoint["model"]['up_block.up_norm.weight']
                del checkpoint["model"]['up_block.up_norm.bias']
                del checkpoint["model"]['up_block.res_block.conv1.weight']
                del checkpoint["model"]['up_block.res_block.conv1.bias']
                del checkpoint["model"]['up_block.res_block.conv2.weight']
                del checkpoint["model"]['up_block.res_block.conv2.bias']
                del checkpoint["model"]['up_block.res_norm.weight']
                del checkpoint["model"]['up_block.res_norm.bias']
                if model_type == "mae":
                    # also change final decoder layer from 3*patch_size*patch_size
                    print("Rescaling final decoder layer for new number of in_chans")
                    target_pix = (
...
def get_model(**hparams):
    ...
    * forward img_size and patch_size args to non sat_mae models
    ...
    elif hparams["model"] == "sat_mae_pp":
        model, _ = get_sat_mae_pp(
            hparams["model_type"],
            pretrained=hparams["pretrained"],
            in_chans=hparams["in_channels"],
            img_size=hparams["input_size"],
    ...
    elif hparams["model"] == "dinov3":
        model = get_dinov3(
            hparams["model_type"],
            pretrained=hparams["pretrained"],
            in_chans=hparams["in_channels"],
            img_size=hparams["input_size"],
            device=hparams["devices"],
            adapter_type=hparams["adapter_type"]
        )
    ...
    # train_prefix = ['up_block', 'proj_up_conv', 'cls_token', 'storage_tokens']
    train_prefix = ['up_block', 'proj_up_conv']
    if hparams["freeze_backbone"]:
        for name, param in model.named_parameters():
            if any(prefix in name for prefix in train_prefix):
                pass
            else:
    ...
    if hparams["adapter"]:
        model = add_adapter(
            ...
            only_proj_trainable=hparams["only_proj_trainable"],
            wavelength=hparams["wavelength"],
        )
    else:
