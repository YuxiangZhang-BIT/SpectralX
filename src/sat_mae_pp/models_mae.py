...
class PatchEmbedUnSafe(PatchEmbed):
    ...
    def forward(self, x):
        ...
        if self.adapter_type == 'spectral_adaptation':
            x, _ = self.proj(x)
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 proj_ratio=4, adapter_type='lora'):
        super().__init__()
        ...
        self.patch_embed = PatchEmbedUnSafe(img_size, patch_size, in_chans, embed_dim, adapter_type=adapter_type)
    ...
    def forward(self, imgs, imgs_up, mask_ratio=0.75):
        ...
        mse_loss = self.forward_loss(imgs, pred, mask)  # MSE loss

        img_dn_2x = F.interpolate(imgs, scale_factor=2, mode='bilinear').to(imgs.device, non_blocking=True)
        pred_2x = self.forward_multiscale(pred)
        ms_loss = torch.nn.functional.l1_loss(pred_2x, img_dn_2x)

        loss = 0.6*mse_loss + 0.4*ms_loss
